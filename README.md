...
- `main.0.15.5.202607241150`
  - 明确瓶装饮用水、牛奶、果汁等可饮用液体属于冰箱库存对象，VLM 应输出 `is_food=true` 和 `category=drink`。
  - 增加瓶装水提示词回归测试，避免模型仅因容器内容为水而按非食物跳过。
  - 增加黄色柑橘外形与叶菜的视觉判别约束，明确有颗粒果皮、无叶片结构的黄色果实应识别为 `柠檬/fruit`，修正真实画面被误判为“小白菜”的问题。
  - 在 NanoPC-T4（RK3399/aarch64）上完成无模拟环境变量的实机终端验收：UVC 照片 `20260724T033811Z.jpg` 经 YOLO 零候选全帧路由进入 Qwen2.5-VL，识别为 `柠檬/fruit`，食品状态与风险保持 `unknown`，本轮 `errors=0`。
  - 实机结果已写入 SQLite，网页 API 返回正常，当前库存为 1、维护告警为 0；终端录像同时保留了主机身份、实时 PID、CPU/内存、传感器、模型输出和数据库同步检查。

- `main.0.15.4.202607232103`
  - 明确 VLM 看到可能食物但无法确定精确子类时仍须输出 `is_food=true`，并使用“肉片”等宽泛名称和较低置信度。
  - VLM 若同时给出明确食物名称、非 unknown 类别和有效组成，但单独把 `is_food` 写成 false，客户端会保留语义结果并标记字段纠正；证据不足的矛盾结果仍拒绝写库。
  - 增加真实肉片响应对应的回归测试，防止“已识别出肉类但因单个布尔字段错误跳过入库”再次发生。
  - 远端复用本轮真实 VLM 响应完成闭环回放：苹果保持原 `food_id`，肉片以新 `food_id` 入库，SQLite 与 Web API 均显示 2 个活跃对象，维护检查告警为 0。
